...
import com.hydracontrolfreak.hcf.hcfdevice.config.EmailConfig;
...
public class HandleEmail implements Runnable {
	...
	@Override
	public void run() {
		HcfDeviceConfig hcfConfig = freak.getHcfConfig();
		...
		EmailConfig emailConfig = hcfConfig.getEmailConfig();
		if (emailConfig == null)
			return;
		...
		for (Integer i : emailActionImpl.getCameraSet()) {
			if (plain) {
				...
				sb.append(parts[1]
						.replaceAll(
								"\\$\\{cameraName\\}",
								encodeHTML(hcfConfig.getCameraConfig().get(i)
										.getName())).replaceAll(
								"\\$\\{url\\}",
