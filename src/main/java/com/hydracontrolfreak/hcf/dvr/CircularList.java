...
//					if (logger.isDebugEnabled()) logger.debug("Dumping circlist");
					...
					String fromFileName = freak.getHcfBase() + "/disk/hcf/tmp_images/" + cam + "/" + imageBuffer.getTimestamp() + ".jpg";
					if (logger.isDebugEnabled()) logger.debug("Linking (" + event.getEventTime() + ") " + fromFileName + " to " +
							freak.getHcfBase() + "/" + imageFilename(event.getDirname(), count));
					linkFile.link(fromFileName, freak.getHcfBase() + "/disk/hcf/" + imageFilename(event.getDirname(), count));
