...
public class EventHeader {
	...
	EventHeader(FreakApi freak, String cam, long eventTime, String description) throws IOException {
		...
		if (logger.isDebugEnabled()) logger.debug("EVENT HEADER: " + freak.getHcfBase());
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
		Date d = new Date(eventTime);
		String dateString = dateFormat.format(d);

		String eventDirname = freak.getHcfBase() + "/disk/hcf/events/" + cam + "/" + dateString;
		...
		File dirFile = new File(freak.getHcfBase() + "/disk/hcf/" + dirname);
		dirFile.mkdirs();
	}
