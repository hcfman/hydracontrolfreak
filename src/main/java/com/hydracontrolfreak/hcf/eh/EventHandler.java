...
public class EventHandler {
	...
	private final HcfDeviceConfig hcfConfig;
	...
	private void startSchedules() {
		...
		for (final Schedule schedule : hcfConfig.getScheduleConfig().getSchedules()) {
			if (logger.isDebugEnabled())
	...
	private void startWatchdogTimers() {
		final Map<String, Watchdog> triggerMap = hcfConfig.getWatchdogConfig().getTriggerMap();
	...
	private void resubscribeToCombinationEvents() {
		...
		final SynthTriggerConfig synthTriggerConfig = hcfConfig.getSynthTriggerConfig();
	...
	private void resubscribeToWatchdogEvents() {
		...
		final WatchdogConfig watchdogConfig = hcfConfig.getWatchdogConfig();
	...
	private void executeAction(final Event event, final Action action, final long eventTime, final boolean reExecute) {
		...
		case ACTION_REMOTE_VIDEO:
			...
			final FreakDevice freakDevice = hcfConfig.getFreakConfig().getFreakMap().get(freakName);
			if (freakDevice == null) {
			...
			sb.append(freakDevice.getProtocol().toString().toLowerCase()).append("://").append(freakDevice.getHostname())
					...
					.append(freakDevice.isGuest() ? "/hcf/guest/remotecam" : "/hcf/remotecam");
		...
		case ACTION_WEB_PREFIX:
			...
			hcfConfig.getSettingsConfig().setWebPrefix(webPrefixActionImpl.getPrefix());
...
//			logger.debug("Not within valid time: " + profile.getTagName());
	...
	private void handleActions(final boolean guest, final Event event, final String eventName, final long eventTime) {
		...
		final ProfileConfig profileConfig = hcfConfig.getProfileConfig();
...
//							logger.debug("Profile (" + profile.getTagName() + ") is now active");
		...
		// for each action
		final ActionConfig actionConfig = hcfConfig.getActionConfig();
		if (logger.isDebugEnabled())
...
//				logger.debug("action.eventName: " + action.getEventName());
	...
	private boolean maySendEvent(final boolean guest, final String eventName) {
		for (final List<HttpTrigger> httpTriggers : hcfConfig.getHttpConfig().getGroupMap().values()) {
	...
	private void handle() {
		final Thread thread = new Thread() {
			private final Map<com.hydracontrolfreak.hcf.eventlib.EventListener, Pattern> eventListenerMap = freak.getListenerMap();
			...
			public void run() {
				...
				while (true) {
					...
					try {
						...
						for (final com.hydracontrolfreak.hcf.eventlib.EventListener eventListener : eventListenerMap.keySet()) {
