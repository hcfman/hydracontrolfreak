...
public class FreakListFetcher {
	...
	public FreakListFetcher(HcfDeviceConfig hcfConfig) {
		...
		if (freakListString != null && freakListString.matches("^(?:\\w+\\:\\d+(?:,\\d+)*)(?:\\s+\\w+\\:\\d+(?:,\\d+)*)*$")) {
			...
			for (String remoteSpecString : freakListString.split("\\s+")) {
				...
				if (matcher.matches()) {
					...
					if (hcfConfig.getFreakConfig().getFreakMap().containsKey(freakString)) {
	...
	public void fetchRemoteCamJSON() {
		...
		for (RemoteCameraSpec remoteCameraSpec : remoteCameraSpecList) {
			...
			try {
				...
				executorService.execute(new HandleRemoteCamFetch(hcfConfig, videoType, remoteCameraSpec));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
