...
//		freak = Freak.getInstance();
    ...
    private boolean initKeyStore(CertificatesJSON certificatesJSON) {
        ...
        try {
            ...
            // Now build up the list of all certificates present, if the "hcf" alias exists, store it's chain json
            for (Enumeration<String> entry = keyStore.aliases(); entry.hasMoreElements(); ) {
                String alias = entry.nextElement();
                if (!keyStore.containsAlias(alias))
                    continue;
    ...
    private boolean initTrustStore(CertificatesJSON certificatesJSON) {
        ...
        KeyStore trustStore = hcfConfig.getCertificateConfig().getTrustStore();
        ...
        if (trustStore == null) {
            ...
            try {
                ...
                hcfConfig.getCertificateConfig().setTrustStore(trustStore);
            } catch (Exception e) {
                ...
                for (Certificate cert : chain) {
                    ...
                    if (cert instanceof X509Certificate) {
                        ...
                        if (trustStore.isKeyEntry(alias)) {
                            Key key = trustStore.getKey(alias, hcfConfig
    ...
    protected void service(HttpServletRequest request,
        ...
        // hcfConfig.getFreakConfig().getFreakMap()
        // .values())
