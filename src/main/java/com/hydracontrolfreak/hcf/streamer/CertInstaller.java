...
public class CertInstaller {
    ...
    public CertInstaller(FreakApi freak, HcfDeviceConfig hcfConfig, String tmpLocation) throws Exception {
    	this.freak = freak;
    	this.hcfConfig = hcfConfig;
		this.tmpLocation = tmpLocation;

		keyStorePassword = hcfConfig.getSettingsConfig().getKeystorePassword();
		trustStorePassword = hcfConfig.getSettingsConfig().getTruststorePassword();
		...
		trustStore = hcfConfig.getCertificateConfig().getTrustStore();
    ...
    public synchronized void importCert(boolean inKeyStore, String alias, String certificateString) throws Exception {
    	...
    	if (!inKeyStore && "hcf".equals(alias)) {
    		oplogger.error("Trying to insert the hcf certificate into the trustStore, this must go into the keyStore");
    		throw new Exception("Trying to insert the hcf certificate into the trustStore, this must go into the keyStore");
    	}
    	
		if ("hcf".equals(alias) && (!keyStore.containsAlias("hcf") || !keyStore.entryInstanceOf("hcf", KeyStore.PrivateKeyEntry.class)))
		...
		try {
			PrintWriter printWriter = new PrintWriter(new FileWriter(tmpLocation + SEP + filename));
			try {
			...
			try {
				...
				File storeFile = new File(freak.getHcfBase() + SEP + (inKeyStore ? ("cacerts" + SEP + "keystore.jks") : ("certs" + SEP + "truststore.jks")));
