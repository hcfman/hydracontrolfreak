...
public class ListJSON implements Serializable {
	...
	private SortedSet<Integer> initialiseCameraSet(HcfDeviceConfig hcfConfig,
		...
		if (cameraListString != null
			...
			for (String s : cameraListString.split(",")) {
				try {
					...
					if (cameraOrder.contains(cam)) {
						if (guest) {
							if (hcfConfig.getCameraConfig().getCameraDevices()
									.get(cam).isGuest())
								cameraSet.add(cam);
						} else
							cameraSet.add(cam);
			...
			cameraSet = hcfConfig.getCameraConfig().getCameraOrder();
		}
	...
	ViewJSON getJSON(boolean fromRemote, FreakApi freak, VideoType videoType,
		...
		cameraDevices = hcfConfig.getCameraConfig().getCameraDevices();
		...
		// Initialise options
		cameraSet = initialiseCameraSet(hcfConfig, request);
		eventFilterList = initialiseEventMap(request);
		FreakListFetcher freakFetcher = new FreakListFetcher(hcfConfig);
		...
		ViewJSON viewJSON = new ViewJSON(freak, totalArraySize, cameraSet,
				eventFilterList);
		viewJSON.filter(startDateStr, endDateStr, times, false, hcfConfig
