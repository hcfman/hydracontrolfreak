...
import com.hydracontrolfreak.hcf.json.ActionJSON;
import com.hydracontrolfreak.hcf.json.ActionsJSON;
...
//		freak = Freak.getInstance();
	...
	private ActionsJSON getActionsJSON() {
		...
		if (logger.isDebugEnabled())
			logger.debug("Setting getAvailableEventNames: "
					+ hcfConfig.getAvailableEventNames());
		actionsJSON.setAvailableEventNames(hcfConfig.getAvailableEventNames());
		actionsJSON.setAvailableProfiles(hcfConfig.getAvailableProfiles());
		actionsJSON.setAvailableTagNames(hcfConfig.getAvailableTagNames());
		actionsJSON.setAvailableFreakNames(hcfConfig.getAvailableFreakNames());
		actionsJSON.setAvailableCameras(hcfConfig.getAvailableCameras());
		actionsJSON.setAvailablePhidgets(hcfConfig.getAvailablePhidgets());
		actionsJSON.setAvailableRfxcomCommands(hcfConfig
				.getAvailableRfxcomCommands());
		actionsJSON.setAvailableButtonGroups(hcfConfig
				.getAvailableButtonGroups());

		fetchRemoteCamListJSON(hcfConfig.getFreakConfig().getFreakMap()
				.values());

		Action[] actionsArray = hcfConfig.getActionConfig().getActionList()
	...
	protected void service(HttpServletRequest request,
		...
		if (logger.isDebugEnabled())
			logger.debug("eventNames: " + hcfConfig.getAvailableEventNames());
